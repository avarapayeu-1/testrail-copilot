...
// Custom metrics
const projectApiCalls = new Counter('project_api_calls');
...
export const options = {
  ...
  thresholds: {
    ...
    project_api_calls: ['count>100'],                   // Should execute more than 100 API calls during the test
  },
};
...
function getProject(projectId: number) {
  const url = buildUrl(`get_project/${projectId}`);
  const params = {
    headers: getAuthHeaders(),
  };
  
  // Track API calls with custom metric
  projectApiCalls.add(1);
...
export default function() {
  // Use project IDs 1-5 randomly to avoid hitting the same project every time
  const projectId = Math.floor(Math.random() * 5) + 1; 
  
  // Call the API
  const response = getProject(projectId);
  
  // Check response
  check(response, {
    'Status is 200': (r) => r.status === 200,
    'Response body has project data': (r) => {
      try {
        const responseBody = r.json();
        return responseBody !== null && typeof responseBody === 'object' && 'id' in responseBody;
      } catch (e) {
        return false;
      }
    },
